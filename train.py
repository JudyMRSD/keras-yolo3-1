...
from yolo3.train_monit import TrainMonitorTools
from yolo3.plot_bbox_training import Plot_Train_Bbox
...
def get_training_data(annotation_path, data_path, input_shape, max_boxes=100, load_previous=False):
    ...
    with open(annotation_path) as f:
        for line in f.readlines():
            ...
            print("filename", filename, "base", base)
            base = os.path.splitext(base)[0]
            boxed_image = letterbox_image(image, tuple(reversed(input_shape)))
            np_boxed_image = np.array(boxed_image,dtype='uint8')
            image_data.append(np_boxed_image)
            ...
            if Plot_Training_Instances == True:
                plotBbox.plot_training_resized(base, np_boxed_image, boxes)
                # cv2.imwrite(Train_Log_Dir + base +"_box.jpg", np_boxed_image)
    ...
    # if Plot_Training_Instances==True:
    #     plotBbox = Plot_Train_Bbox()
    #     plotBbox.plot_training_instances(image_data, box_data)
